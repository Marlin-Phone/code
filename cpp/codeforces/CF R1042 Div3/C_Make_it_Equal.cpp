...
void solve() {
    ...
    vector<int> s_norm, t_norm;
    for (int i = 0; i < n; i++) {
        int r = s[i] % k;
        int r_prime = min(r, k - r);
        s_norm.push_back(r_prime);
    }
    for (int i = 0; i < n; i++) {
        int r = t_arr[i] % k;
        int r_prime = min(r, k - r);
        t_norm.push_back(r_prime);
    }

    sort(s_norm.begin(), s_norm.end());
    sort(t_norm.begin(), t_norm.end());
